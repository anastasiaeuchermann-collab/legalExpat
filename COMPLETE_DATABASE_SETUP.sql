-- =============================================
-- LEGALEXPAT - COMPLETE DATABASE SETUP
-- =============================================
...
-- 2. Select your project: kcxzbbbpbqrwmznkclxj
...
-- =============================================
-- MIGRATION 1: CREATE USERS AND PROFILES
-- =============================================
...
-- =============================================
-- MIGRATION 5: CREATE REVIEWS AND MESSAGES
-- =============================================
...
-- Note: RLS policies will be defined in migration 9 with NextAuth integration
...
-- =============================================
-- MIGRATION 9: UPDATE RLS FOR NEXTAUTH
-- =============================================

-- Create auth context table
...
CREATE POLICY "Anyone can view active services"
    ...
    USING (is_active = true OR provider_id = current_user_id());
...
CREATE POLICY "Providers can manage own pricing tiers"
    ...
    USING (
        EXISTS (
            ...
            AND s.provider_id = current_user_id()
        )
    );
...
CREATE POLICY "Users can view booking documents"
    ...
    USING (
        current_user_id() = uploaded_by OR
        EXISTS (
            SELECT 1 FROM bookings b
            WHERE b.id = booking_documents.booking_id
            AND (b.expat_id = current_user_id() OR b.provider_id = current_user_id())
        )
    );

CREATE POLICY "Users can upload booking documents"
    ON booking_documents FOR INSERT
    WITH CHECK (current_user_id() = uploaded_by);
...
CREATE POLICY "Anyone can view published reviews"
    ...
    USING (status = 'published' OR expat_id = current_user_id() OR provider_id = current_user_id());
...
CREATE POLICY "Users can view own message threads"
    ...
    USING (current_user_id() = expat_id OR current_user_id() = provider_id);
...
CREATE POLICY "Users can view messages in their threads"
    ...
    USING (
        current_user_id() = sender_id OR current_user_id() = receiver_id
    );

CREATE POLICY "Users can send messages"
    ON messages FOR INSERT
    WITH CHECK (
        current_user_id() = sender_id AND
        EXISTS (
            SELECT 1 FROM message_threads mt
            WHERE mt.id = thread_id
            AND (mt.expat_id = current_user_id() OR mt.provider_id = current_user_id())
        )
    );
...
-- Next steps:
-- 1. Test the connection by running: SELECT COUNT(*) FROM users;
-- 2. Create test accounts through the registration form
